...
> The function above spit out the integers included in x, but it skipped over/excluded the numbers in the 3rd and 1st postions (2 and 15.4 respectively). 
...
> The script beginning on line 63 is only printing the integers that assigned a 'true' value. As it reads true, false, true, true, false, the second and fifth integers (25 and 5) are excluded. After playing with this function in the terminal, I discovered that it works as a loop. Meaning if you assign x[c(TRUE, FALSE)], it will print out every other integer until every integer assigned to x is accounted for. The script beginning in line 69 is asking for the computer to return every number in x that is greater than 7. 
...
Immediately, y appeared in the environment and reads "Named num [1:7]," followed by the integers that compose x. The first 4 numbers return named "a" "b" "c" and "d" because the first line of code told the computer return all of the integers in x, as well as the first four letters of the alphabet. That is why the 5th, 6th, and 7th integers in x are not accompanied by a letter. Line 86 confirms that the previous script worked, as 8.35 is synonymous with "d" in y. 
...
> The second example provides only the first 3 rows, but all of the columns. The first example provides only columns 2 and 3 but all of their rows. 
...
> I wanted to subset each judge's gender and ethnicity. I anticipated that the results would be majority male and white. Isolating these columns makes it easier to look at the information that is relevant to my curiosity. 
...
> This code provides the data stored in the city column for rows 100 through 110. 
...
```{r}
p <- c("Jefferson", "Lincoln", "Bush", "Carter", "Coolidge")
length(p)
if (length(p) > 5) {"yes"} else {"no"}
```
...
```{r}
max(gayguides$Year)
if (max(gayguides$Year) > 1990) {"true"} else {"false"}
```
...
> I remebered from last week that you can check the latest year using the max function. I hit a snag creating the if statement because I tried to simply use gayguides$Year > 1990, which was spitting back an error. I decided to try it with the max function and it worked! I had the pieces correct, I just needed a little trial and error to put it together properly. 
...
> length() indicates that the code is to work through each item in presidents. We use it on presidents to create the loop that it is supposed to shuffle through. 
...
```{r}
classes <- c("Hist 8000", "Hist 8520", "Hist 8510")
for (c in 1:length(classes)) {
  print(paste("I am enrolled in", classes))
}
```
...
The loop only printed 2 and 3 because it was told to skip anything less than 3 and it stopped after 4 because 5 is greater than OR EQUAL TO 5. 
...
```{r}
for (c in 1:length(statepopulations$X1800)) {
if (is.na (statepopulations$X1800[c])){
  next
}
    else if (statepopulations$X1800[i] < 200000){
       next
}
   else if (statepopulations$X1800[c] > 250000){
          next 
   }
  
  else 
   print(paste(statepopulations$STATE[c], "is between 200000 and 250000")) 
...
#1:length(statepopulations$X1800)

for (c in 1:length(statepopulations$X1800)) {
  if (!is.na (statepopulations$X1800[c] > 200000) | 
      !is.na(statepopulations$X1800[c] < 250000))
    print(paste(statepopulations$STATE[c], "is between 200000 and 250000")) 
  next
}
...
```{r}
for (c in 1:length(gayguides$city)) {
  #print(gayguides$city[c]) } 
  
  if (gayguides$city[c] == "Greenville") {
    print(gayguides$title[c])} } 
```
...
historyStudents(studentname, class)
...
```{r}
myfirstfunction <- function(z) {
  z <- "This is a message and I really hope this works."
  myvalue <- nchar(z)
  statement <- paste("There are ", myvalue, "characters in that string.")
  statement
  }
...
> That was so cool! First, I had to google the function that counts characters (nchar). Once I figured that out, I used the previous function as a template and changed what I needed to. I hit a snag when I assigned my string as follows - myfirstfunction <- function("This is a message and I really hope this works.") When I received an error messaged, I ran to google, and, after looking at some examples, I settled on assigning a value to the function, and then assigning my string to that value. It worked! 
...
> This is because you are assigning grade(60), aka "C," to George Washington as a variable. It appears in the environment rather than being printed out. 
...
GayGuidesLocations <- function (city, year) {
  data(gayguides)

  for (x in 1:length(gayguides$city)) {
    if (gayguides$city[x] == city && gayguides$Year[x] == year) {
     print(paste("Found a location called ", gayguides$title[x]))
    } else {
      next 
    } 
  }
...
GayGuidesLocations("Washington", 1983)
...
```{r}
BotsonWomenVotersOccupation <- function(Occupation) {
  #data(BostonWomenVoters)

  for (x in 1:length(BostonWomenVoters$Occupation)) {
    if (BostonWomenVoters$Occupation[x] == Occupation) {
     print(BostonWomenVoters$Name[x])
    } else {
      next 
    } 
  }
}
BotsonWomenVotersOccupation("Student") 
...
#m <- mat[BostonWomenVoters[,"Occupation"]=="x",]

  for (x in 1:length(BostonWomenVoters$Occupation)) {
    if (BostonWomenVoters$Occupation[x] == Occupation) {
   print(BostonWomenVoters[x,])
    } else {
      next 
    } 
  }
...
for (p in 1:length(statepopulations$X1800)) {
  ...
  else if (statepopulations$X1800[p] < 200000) {
    next
  }
  else if (statepopulations$X1800[p] > 250000) {
    next
  }
