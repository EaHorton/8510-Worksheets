...
SEE SEPARATE PROJECTS FOR MIND BODY AND BUFFALO BILL
...
```{r}
#| eval: false
data_dir_new <- paste(getwd(), "/txt", sep = "")
mb <- readtext(paste0(data_dir, "/*.txt"))
...
mb_whole <- 
  mb.metadata %>%  
  arrange(Filename) %>% # sort metadata
  bind_cols(mb_new) %>% # combine with texts
  as_tibble() # convert to tibble for better screen viewing

#mb_update <- mb_whole %>% mutate(Filename = paste("MB_",Filename...1))
...
tidy_mb_words <- tidy_mb %>% count(Filename...8, word) #correct up until this step

mb.dtm <- tidy_mb_words %>% 
  count(word, Filename...8) %>% 
  cast_dtm(word, Filename...8, n)
...
#dtm <- DocumentTermMatrix(tidy_mb)
#lda_model <- LDA(dtm, k = 25)
...
#mb.topics <- tidy(lda_model, matrix = "beta")
head(mb.topics)
...
custom_stopwords <- c("january", "february", "march", "april", "may", "june", "july", "august", "september", "october", "november", "december", )
stopwords <- c(stopwords, custom_stopwords)
dtm <- DocumentTermMatrix(tidy_mb, control = list(stopwords = TRUE))
lda_model <- LDA(dtm, k = 25)
...
mb_1920 <- tidy_mb %>% filter(Year == 1920) %>% select(Year, Month, OCR, Volume, No, Notes, Filename...8, word)
...
#url <- "https://github.com/dseefeldt/IndianaNewspapers/raw/main/bb-txt.zip"
#download.file(url = url, destfile = "./bb-txt.zip")
#unzip(zipfile = "./bb-txt.zip", exdir = "./bb-txt")
...
data_dir_bb <- paste(getwd(), "./bb-txt", sep = "")
bb <- readtext(paste0(data_dir_bb, ".bb-txt"))
