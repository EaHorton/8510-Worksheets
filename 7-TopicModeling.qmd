...
```{r}
dtm <- DocumentTermMatrix(mb.metadata)
...
custom_stopwords <- c("editorial", "committee", "to", "digitized", "by", "review", "google")
stopwords <- c(stopwords, custom_stopwords)
dtm <- DocumentTermMatrix(mb.metadata, control = list(stopwords = TRUE))
lda_model <- LDA(dtm, k = 10)
