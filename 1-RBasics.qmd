...
```{r}
paste("I need to think about", "committing these changes soon")
```

(@) Can you explain what happened in 2-3 sentences? 

> The paste function took two separate words or phrases (aka strings) and it put them together as a single string of text. It is essentailly adding them together, however it does not create a new unit as a result - meaning you can see the elements that were pasted together. 
...
> Row 96 asks if 5 is equal to 10, which is false. Row 97 asks if 3 is less than 4, it is. 98 asks if 5 does not equal 10. 99 asks if 5 does not equal 5. Row 100 asks if 3 is equivalent to 4 or 3, it is true because 3 equals 3 even if it does not equal 4. Row 101 asks if 3 does not equal 4 AND 3 does not equal 5. 
...
> In the code above, each number included in the my.numbers variable was multiplied by 2. 2*2=4, 4*2=8, 6*2=12, and 8*2=16. It deals with each number in the variable individually.   
...
> In the code above, the third number in the my.numbers varaible, which is 6, was multiplied by 2. 6 multiplied by 2 equals 12. 

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
grocery.list <- c("eggs","milk", "oatmeal", "chocolate", "cinnamon")
```
...
> Floor takes a single numeric argument x and returns a numeric vector containing the largest integeres not greater than the corresponding elements of x. 
...
> The third column (z) takes the integer from column x, multiplies it by 2, and adds the integer in column y (which is always 1 in this case). Column z only stores the solution to the equation. For example row one is 1 * 2 + 1 = 3 or x * 2 + y = z. 
...
> This dataset contains information from 1965 to 1985. 
...
> For the first prompt, I attempted to use the ? prompt in the consul to figure it out. However, after a couple moments of searching, I decided to Google it. I didn't even have to click on a link before I found my answer. For the second prompt, I simply replicated the head() function above with tail instead of head (per the help guide), assuming it followed the same principle of a default 6 rows. 
...
> gayguides is the name of the dataset, $ is what allows us to get a segemnt of the data, title is the descriptor of the row we're searching, and 500 is the specific row we are looking for. 
...
> I learned that this dataset is tracking the populations of more than just offical states. It also accounts for territories and persons in the military. This dataset is organized differently than the gayguides dataset, as the year is a demarkated in the column rather than a varibale - so the max and min functions do not work for the year in this context. The years included in this dataset are every-ten-years from 1790 to 2020. There are 28 columns and 84 rows. 
