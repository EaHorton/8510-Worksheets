...
> I assigned the script to a function entitled GG so that R knew to store it somewhere. 
...
```{r}
gayguides %>% filter(Year >= 1975 & Year <= 1980) %>%  filter(city == "Greenville" & state == "SC")
```

(@) Can you find all locations in 1975 except for New York and San Francisco? 
```{r}
gayguides %>% filter(Year == 1975) %>%  filter(city != "New York" & city != "San Francisco")
```
...
gayguides %>% filter (str_detect(amenityfeatures, '(G)|(L)'))

#mpg_df %>% filter(cyl %in% c(4,5,6))
#gayguides %>% select(contains("(G)" | "(L)"))
...
```{r}
gayguides %>% select(city, state) %>% mutate(location = paste0(city,",", state))
...
```{r}
statepopulations %>% select(STATE, X1850) %>% 
  arrange(desc(X1850))
```
...
#gayguides %>% select(description, city, Year, state) %>% filter(state == "SC") %>% filter(Year >= 1970 & Year <= 1975) %>% group_by(city) %>%  summarize(count = n())

avg.by.year <- gayguides %>% filter(state == "SC") %>% filter(Year == 1970:1975) %>% group_by(city, Year) %>% summarize(count = n()) %>% summarize(avg = mean(count))


#gayguides %>% select(description, city, Year) %>% group_by(Year) %>%summarize(count = n())
```
...
#gayguides %>% select(description, city, Year, state) %>% filter(state == c("AL", "AR", "FL", "GA", "KY", "LA", "MD", "MS", "NC", "SC", "TN", "TX", "VI", "WV")) %>% filter(Year == 1975) %>% group_by(Year) %>%  summarize(count = n())


#in class way of doing this 
gayguides %>% select(description, city, Year, state) %>% filter(state %in% southernstates) %>% filter(Year == 1975) %>% group_by(Year) %>%  summarize(count = n())
...
census_regions_info <- left_join(gayguides, regions, by=c("state"="State.Code"))

census_regions_info %>% select(Region, Year, title, Division) %>% filter(Year == 1980) %>%  group_by(Division, Region) %>%  summarize(count = n())
...
I joined state with State.Code because they corresponded in content and format. After I joined the datasets, I used the filter, group_by, and summarize features that we learned above. The mountain region is contained in Division rather than region, but I included both, which works because all Mountain entries are in the West. The total number is 285. 
...
census_regions_info %>% select(Region, Year, State) %>% filter(Year == 1980 | Year == 1970) %>%  group_by(State, Region) %>% mutate(growth = (count = n()))
...
> This code took the years that were provided originally in columns and turned them into rows.This lengthened the dataset.  
...
```{r}
#Looking chronologically, when do we start to see women appearing in the dataset and at what frequency?
```
...
```{r}
judges %>% 
  #go to judges dataset
    select(Confirmation.Date..1., Gender)  %>% 
  #select the two columns that are relevant to my question
arrange(desc(Confirmation.Date..1.)) %>%
  #arrange the dates in descending order so that I can view the data chronologically 
    group_by(Confirmation.Date..1., Gender) %>%
  #in my result, I want to see both the date and gender of the judge
    summarize(count = n())
  #i want to know how many judges are initiated on each date in the case that there are multiple
```

(@) Now ask a question of the `gayguides` data (or another dataset of your choice). What is the question you are asking? 
```{r}
#How many street addresses are located on an interstate or highway? 
```

(@) Now write some code to address that question. Comment the code with notes that explain your thinking as you go. Use functions like select(), filter(), etc to pair down your dataset and reshape it to address your question. 
```{r}
gayguides %>% select(streetaddress, Year, state) %>%
# these are the columns that I want to appear in my final results
filter (str_detect(streetaddress, 'Hwy.| I-')) %>%
# I need R to identify any address with Hwy or I in the name
group_by(Year, state) %>%
  #I don't need the actual street address to appear in the final result, but I do want the count. 
  summarize(count = n())
...
```{r}
gg_function <- function(state1, Year1){
  gayguides %>% select(Year, state, title, type) %>%
  filter(state == state1) %>% filter(Year == Year1)}
