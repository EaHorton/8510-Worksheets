...
rec.workers.1. <- rec.workers %>%
 pivot_longer(X1930:X1945, names_to = "year", values_to = "count")  %>% pivot_wider(names_from = type_of_worker, values_from = count) %>% pivot_longer("male paid":"female paid", names_to = "paid", values_to = "count_paid") %>% pivot_longer("men volunteer":"women volunteer", names_to = "men volunteer", values_to = "count volunteer")  
...
ggplot(judges.month, aes(x = Birth.Month, y = count)) +
  geom_col() + labs(x="Birth Month", y="Count", title="Birth Month of US Judges and Justices")
...
ggplot(judges.gender, aes(x = Birth.Year, y = count)) +geom_line() + facet_wrap(~Gender) + geom_col()
```
...
```{r}
BWO <- BostonWomenVoters %>% filter(Occupation=="Housekeeper") %>% group_by(Occupation, Age) %>% summarize(count = n()) 
